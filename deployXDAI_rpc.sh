...
FOREIGN_XDAI_PROXY=$(forge create --rpc-url $SOURCE_RPC_URL --private-key $DEPLOYER_PRIV_KEY -—verify -—verifier $SOURCE_VERIFIER —-etherscan-api-key $SOURCE_API_KEY tokenbridge-contracts/contracts/upgradeability/EternalStorageProxy.sol:EternalStorageProxy --json)
...
FOREIGN_XDAI_IMPLEMENTATION=$(forge create --rpc-url $SOURCE_RPC_URL --private-key $DEPLOYER_PRIV_KEY -—verify -—verifier $SOURCE_VERIFIER —-etherscan-api-key $SOURCE_API_KEY tokenbridge-contracts/contracts/upgradeable_contracts/erc20_to_native/XDaiForeignBridge.sol:XDaiForeignBridge --json)
...
FOREIGN_XDAI_UPGRADE_TX=$(cast send $FOREIGN_XDAI_PROXY_ADDRESS "upgradeTo(uint256, address)" 1 $FOREIGN_XDAI_IMPLEMENTATION_ADDRESS  --rpc-url $SOURCE_RPC_URL --private-key $DEPLOYER_PRIV_KEY --json)
...
HOME_XDAI_PROXY=$(forge create--rpc-url $DEST_RPC_URL --private-key $DEPLOYER_PRIV_KEY  -—verify —-verifier $DEST_VERIFIER —-etherscan-api-key $DEST_API_KEY $VERIFIER_URL tokenbridge-contracts/contracts/upgradeability/EternalStorageProxy.sol:EternalStorageProxy --json)
HOME_XDAI_PROXY_ADDRESS=$(jq -r '.deployedTo' <<< "$HOME_XDAI_PROXY")
echo "Home xDAI Proxy deployed to " $HOME_XDAI_PROXY_ADDRESS

HOME_XDAI_IMPLEMENTATION=$(forge create --rpc-url $DEST_RPC_URL --private-key $DEPLOYER_PRIV_KEY  -—verify —-verifier $DEST_VERIFIER —-etherscan-api-key $DEST_API_KEY $VERIFIER_URL tokenbridge-contracts/contracts/upgradeable_contracts/erc20_to_native/HomeBridgeErcToNative.sol:HomeBridgeErcToNative --json)
...
HOME_XDAI_UPGRADE_TX=$(cast send $HOME_XDAI_PROXY_ADDRESS "upgradeTo(uint256, address)" 1 $HOME_XDAI_IMPLEMENTATION_ADDRESS  --rpc-url $DEST_RPC_URL --private-key $DEPLOYER_PRIV_KEY --json)
